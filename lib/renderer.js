...
let baseDir;
function getBaseDir(sub) {
  if (!baseDir) {
    if (require.main) {
      baseDir = require.main.filename;
...
class FloofRenderer {
  ...
  recompile() {
    ...
    const templateDir = getBaseDir('templates');
    for (const file of crawlDir(templateDir)) {
      ...
      files.set(file.substring(templateDir.length + 1), {tokens, extend});
    }
    ...
    for (let [name, {tokens, defines, extend}] of cleanedFiles) {
      ...
      function resolveDef(id) {
        ...
        let defineMap = defines;
        let parentName = extend;
        while (!defineMap[id] && parentName) {
          defineMap = cleanedFiles.get(parentName);
          parentName = defineMap.extend;
          defineMap = defineMap.defines;
        }
        const tokens = defineMap[id];
        if (!tokens) return '';
...
module.exports = {FloofRenderer, getBaseDir};