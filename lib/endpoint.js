...
class WrappedEndpoint {
  ...
  async render(req, path, pathParams, params) {
    ...
    wrapped = new Proxy(wrapped, {
      ...
      get(target, key) {
        return target[key] || pathParams.get(key) || parsedParams.get(key);
      },
    });
...
class EndpointRegistry {
    ...
    if (endpoint) {
      for (const error of endpoint.parent.errors) {
        if (error.matches(req.statusCode)) {
          return await this.renderError(error, req, msg, endpoint.parent, endpoint);
        }
      }
    }
      ...
      if (error.matches(req.statusCode)) {
        ...
        req.delegateError = async function(floofball) {
          for (const error of floofball.errors) {
            if (error.matches(req.statusCode)) {
              return await self.renderError(error, req, msg, floofball);
            }
          }
          return msg;
        };
  ...
  async renderError(error, req, msg, floofball, ept = fakeEndpoint) {
    const {path, query} = EndpointRegistry.parsePrelim(req.url);
    let wrapped = new FloofRequest(ept, req, path, emptyMap, query);
    wrapped = new Proxy(wrapped, {
      has(target, key) {
        return target.hasOwnProperty(key) || query.has(key);
      },
      get(target, key) {
        return target[key] || query.get(key);
      },
    });
    const renderer = this.floof.renderer.contextualize(floofball, wrapped);
    await floofball.befores.run(wrapped);
    let response = await error.executor(wrapped, msg, renderer);
    if (!(response instanceof Stoof)) response = new Stoof(wrapped.statusCode, response);
    await floofball.afters.run(wrapped, response);
    return response;
  }
